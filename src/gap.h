...
inline int c_clip_extend_gap2_(String<uint64_t> & hs, 
	...
	)
{
    ...
    int itsEnd = 0;
    int drop_count = 0;
    uint64_t pre_anchor = anchors[n - 1];
    ...
    /*
    for (int i = n - 1; i > 0 ; i++)
    {   //process the first block in anchors
        short x = short(g_hs_anchor_getX(anchors[i]));
        short y = short(g_hs_anchor_getY(anchors[i]));
        short delta_x = itEnd_genome - itBegin_genome - x;
        short delta_y = itEnd_read - itEnd_read - y;
        ...
        std::cout << "mergexxx " << ex_y[tmp_n - 1] << " " << ex_x[tmp_n - 1] << " " << delta_x << " " << delta_y << " " << i << "\n";
        if (g_hs_anchor_getY(pre_anchor ^ anchors[i]))
        {
            if (hs_len_read - g_hs_anchor_getY(anchors[i]) > thd_gap_len)
            {
                return 0;
            }
            if (tmp_n > 0)
...
{   
    ...
    int step = c_shape_len3;
    for (int j = 1; j < itEnd_read - itBegin_read + 1; j++)
    {
        int flag = 1;
                            ...
                            )
{
    ...
    {
        std::cout << "[]::anv << " << (int64_t)(g_hs_anchor_getAnchor(anchor[k]) - gs_start + gr_start) << " " << int64_t(g_hs_anchor_getX(anchor[k]) - gs_start) << " " << int64_t(g_hs_anchor_getY(anchor[k]) - gr_start) << "\n";
        uint64_t dx = std::abs(g_anchor_dx_(anchor[k], anchor[k + 1]));
        uint64_t dy = g_hs_anchor_getY(anchor[k + 1] - anchor[k]);
        int64_t da = g_anchor_da_(anchor[k], anchor[k + 1]);
        if (da == 0 && dy == 1) 
    ...
    for (int i = 0; i < it - 1; i++)
    {
        ...
        for (int j = i + 1; j < it - 1 && width_count < thd_width; j++)
        {
            ...
            if (std::abs(d_anc) < std::max(int(x0 - x1) >> thd_merge1, thd_merge1_lower) && (x0 - nxt_end) < thd_merge2 && x0 > x1)
            {
                ...
                std::cout <<"[]::axit " << x1 << " " << y1 << " " << d_anc << " conts " << c_conts << " " << x1 - nxt_end << " " << nxt_c_conts << " " << x1 - x0 << " "<< sc << "\n";
                x0 = x1; y0 = y1;
                x_end = x1;
                y_end = y1;
                ++dj;
