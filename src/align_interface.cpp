                   ...
                   )
{
    ...
    std::cout << "cigar_before " <<  bam_record.beginPos << " " << lens.first << " " << lens.second << "\n";
    //>>debug
    if (pos < 0)
    {
        printCigar(bam_record.cigar, "ib0");
        align2cigar(bam_record.cigar, row1, row2);
        printCigar(bam_record.cigar, "ib00");
        dout << "ib2" << bam_record.beginPos << "\n";
    }
    else
    {
        ...
        std::cout << "inscigar " << length(tmp) << bam_record.beginPos << "\n";
        printCigar(tmp, "ib1");
        dout << "ib2" << bam_record.beginPos << "\n";
    }
    //<<debug
                      ...
                      )
{
    ...
    else
    {
        ...
        {
            printCigar(bam_record.cigar, "insertBamRecord1");
            insertBamRecordCigar(bam_record, row1, row2, 1);
            cigar[0].count = r_beginPos;
            cigar[0].operation = op;
            printCigar(bam_record.cigar, "insertBamRecord2");
        }
        else
        {
				 ...
				 )
{
    ...
    {
        std::cout << "r11 " << get_cord_y(cord1) << " " << get_cord_y(cord2) << " " << endPosition(row11) << " " << beginPosition(row21) + delta1 << "\n";
        return 1;
    }
	...
	for (int i = 0; i < length(align1) - 1; i++)	
	{
		...
		for (int j = start_j; j < length(align2); j++)	
	    {
            ...
            //printRows(row11, row12, "row11x");
            //printRows(row21, row22, "row12x");     
            if (dx + dy <= 1 && dx >=0 && dy >= 0)
			{
                ...
                if (dy == 1)
                {
                    std::cout << "dxy1 " << get_cord_x(cord1) << "\n";
                    insertGap(row11, clip1 - clippedBeginPosition(row11) - 1);
                    clip1++;
                }
                else if (dx == 1)
                {
                    std::cout << "dxy2 " << get_cord_x(cord1) << "\n";
                ...
                {
                    dout << "dxy0 " << get_cord_x(cord1) << dx << dy << get_cord_x(cord2) - get_cord_x(cord1) +clippedEndPosition(row11) << clippedBeginPosition(row21) << "\n";
                }
                //>>debug
                ...
                //printRows(row11, row12, "row11xx");
                //printRows(row21, row22, "row12xx");                
				return 0;
			}
              ...
              )
{
    ...
    for (int k = 0; k < clipped_len; k++)
    {
        ...
        if (count++ == delta)
        {
            ...
            //dout << "v2" << src1 << toSourcePosition(row1, buf_len * delta) << "\n";
            buf_len++;
            count = 1;
        }
        ++it1;
...
//TODO::score to accecpt the segs, need parm tunning 
            if (dens_ > thd_accept_density)
            {
                    ...
                    clip_pair.first - last_clip > thd_min_interval)
                {
                    ...
                    ++ct_clips;
                    dout << "clpr1" << prebp * delta << toSourcePosition(row1, prebp * delta) << buffer_src1[prebp] << clip_pair_src1.first << "\n";
                }
                else
                {
               ...
               GapParm & gap_parm)
{
    ...
    if (length (clips) == 1)
    {
        ...
        //<<debug
        std::pair<int, int> tmp = countCigar(bam_records[bam_id].cigar);
        //>>debug
        printRows(row1, row2, "1kd");
        addNextBamLink (bam_records, bam_id, bam_next_id);
        std::cout << "ag3 " << " " << get_cord_y(str_cord) << " " << bam_id << " " << bam_next_id << length(bam_records) << " " << clips[0].first << " " << clips[0].second << " " << bam_records[bam_id].beginPos + tmp.first << "\n";
    }
    else if (length(clips) > 1)
    {
        dout << "lenrow" << end(row1) - begin(row1) << back(clips).first << length(row2) << "\n";
        ...
        dout << "agx4" << length(clips) << clips_src1[0].first << clips_src1[0].second << back(clips_src1).first << back(clips_src1).second << bam_start_x << "\n";
        for (int i = 1; i < length(clips) - 1; i++)
        {
            ...
            dout << "algs1 " << bam_start_x << bam_start_y << "\n";
        }
        ...
        for (int i = 0; i < length(clips) - 1; i++) 
        {
            ...
            for (int j = 0; j < length(seg_clips); j++)
            {
                ...
                insertNewBamRecord(bam_records, row1, row2, g_id, bam_start_x, bam_start_y, bam_strand); 
                dout << "ac4 " << seg_clips_src1[j].first << seg_clips_src1[j].second << seg_clips[j].first << "\n";
                std::cout << "ac4 " << genomes[0][bam_start_x] << "\n";
                //printRows(row1, row2, "ac4");
            }
        }
    }
...
// return 1 for same strand cord if the pre-cord is bigger
int check_cord_2_(uint64_t cord1, uint64_t cord2)
{
    if (!_DefaultCord.isBlockEnd(cord1) && 
        !get_cord_strand(cord1 ^ cord2))
    {
        if (get_cord_y(cord1) > get_cord_y(cord2) || 
            get_cord_x(cord1) > get_cord_x(cord2))
        {
            std::cout << "ac_cerr " << cord1 << " " << get_cord_y(cord2)<< "\n";
            return 1;
        }
    }
    return 0;
                ...
                ) 
{
    ...
    //<<debug
    for (int i = 0; i < length(cords_r); i++)
    {
        dout << "acs" << i << get_cord_x(cords_r[i]) << _DefaultCord.isBlockEnd(cords_r[i]) << "\n";
    }
    //>>debug
                 ...
                 !get_cord_strand(back(cords) ^ cords_r[i]))
        {
            ...
            if (cordx1 > cordx2 || cordy1 > cordy2) 
            {
                int64_t danchor =  std::abs(cordx1 - cordy1) -
                    ...
                    dout << "skip" << thd_err_rate * std::abs(cordx1 - cordx2) << std::abs(danchor) << "\n";
                if (std::abs(danchor) > 
    ...
    {
        dout << "acs2" << i << get_cord_y(cords[i]) << get_cord_x(cords[i]) << _DefaultCord.isBlockEnd(cords[i]) << "\n";
    }
    //>>debug
    int f_gap_merge = 0;
    int cblock = 0;
    for (int i = 1; i < (int)length(cords); i++)
    {
        ...
        //debug
        dout << "prex2" << i + 1 << get_cord_y(pre_cord_str) << get_cord_y(cord_str) << "\n";
        check_flag = 0;
        ...
        if (_DefaultCord.isBlockEnd(cords[i - 1])) 
        {
            ...
            check_flag = -1;
            std::cout << "ac2 " << get_cord_y(cord_str) << " " << get_cord_y(cord_end) << " " << dy << " " << dx << " " << get_cord_y(cords[i]) << "\n";
        }
        ...
        else if (get_cord_strand(cords[i] ^ cords[i + 1]))
        {
            ...
            check_flag = 1;
            std::cout << "ac_xx " << get_cord_y(cord_str) << " " << dy << " " << dx << "\n";
        }
        ...
        //<<debug
        dout << "ag_cord_str" << get_cord_x(cord_str) << get_cord_x(cord_end) << " " <<  _DefaultCord.isBlockEnd(cords[i]) << "\n";
        //printRows(rstr[ri_pre], rstr[ri_pre + 1], "ac2_2pre");
        printRows(rstr[ri], rstr[ri + 1], "ac2_2");
        dout << "cords[i]" << get_cord_y(cords[i]) << " " << get_cord_x(cords[i]) << "\n";
        String<CigarElement<> > tmpcigar;
        align2cigar(tmpcigar, rstr[ri], rstr[ri + 1]);
        printCigar(tmpcigar, "tm");
        //>>debug
        ...
        if (flag)
        {
            ...
            */
            ...
            dout << "flag1 " << i + 1 << flag_pre << flag << _DefaultCord.isBlockEnd(cord_str) << get_cord_x(cord_str) << get_cord_y(pre_cord_str) + beginPosition(rstr[ri_pre + 1]) << "\n";
        ...
        if (_DefaultCord.isBlockEnd(pre_cord_str))
        {
            ...
            std::cout << "flag2 " << i + 1 << " " << flag_pre << " " << flag << " " << get_cord_x(pre_cord_str) <<"\n";
            flag = 0;
            flag_pre = 0;
            std::swap (ri, ri_pre); 
            continue;
        ...
        dout << "flag " << i + 1 << get_cord_y(cords[i]) << get_cord_x(cords[i]) << flag_pre << flag << "\n";
        if (!flag_pre)
        {
            if (!flag)
            {
            ...
            else if (flag & 1)
            {
                gap_str_cord = shift_cord(pre_cord_str,
                                          beginPosition(rstr[ri_pre]),
                                          beginPosition(rstr[ri_pre + 1]));
                std::cout << "ac12 " << i + 1 << " " << get_cord_y(gap_str_cord) << " " << get_cord_y(pre_cord_str) << " " << get_cord_x(gap_str_cord) << " " << get_cord_x(gap_end_cord) << "\n";
            }
            else if (flag & 2)
            {
        ...
        else if (flag_pre & 1)
        {
            if (!flag)
            {
                ...
                dout << "ac22 " << i + 1 << get_cord_y(cords[i]) << get_cord_y(gap_str_cord) << get_cord_y(gap_end_cord) << get_cord_x(gap_str_cord) << get_cord_x(gap_end_cord) << get_cord_x(cord_str) + beginPosition(rstr[ri]) << "\n";
                ...
                {
                //<<debug
                    std::cerr << "\nacs_o11 " << get_cord_y(cords[i]) << " " << get_cord_x(cords[i]) << " " << get_cord_y(gap_str_cord) << " " << get_cord_y(gap_end_cord) << "\n\n";
                //>>debug
                }
            }
            else if (flag & 1)
