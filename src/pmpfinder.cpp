...
using std::cout;
using std::endl;
//Note: Register in PMPParms() and PMPParms:toggle for every new parm
PMPParms::PMPParms()
{
    GetDHitListParms();
    GetHIndexMatchAllParms();
    GetDIndexMatchAllParms();
    GetSIndexMatchAllParms();
    ChainAnchorsHitsParms();
    ApxParms();
}
void PMPParms::toggle(int i)
{
    pm_gdl.toggle(i);
    pm_ghima.toggle(i);
    pm_gdima.toggle(i);
...
int extendWindow(FeaturesDynamic & f1, 
                  FeaturesDynamic & f2, 
 ...
 * NOTE::if failed should return new_cord = 0
 */
 ...
 * 1.Drop too short blocks in cords 
   2.remove dxdy < 0 while within map error cord
 * @thd_map_error: expected error(deviation) of map +/-50bps by default
 */
int clean_blocks_ (String<uint64_t> & cords, uint64_t thd_drop_len, int64_t thd_map_error = 50) 
{
    ...
    {
        len++; //length of block after removing unnormal but within error of map elements(dxdy<0, error<50)
        if (!is_cord_block_end(cords[i - 1]))
        {
            int64_t dx = get_cord_x(cords[i]) - get_cord_x(cords[ptr - 1]);
            int64_t dy = get_cord_y(cords[i]) - get_cord_y(cords[ptr - 1]);
            if (dx < 0 || dy < 0) 
            {
                if (std::abs(dx) < thd_map_error && 
                    std::abs(dy) < thd_map_error)
                {
                    --len;
                    --ptr;
                }
...
{
    if (i < (int)length(thd_alphas) && i >= 0)
    {
        thd_alpha = thd_alphas[i];
    }
}
unsigned getSIndexMatchAll (SIndex & index,
                            String<Dna5> & read,
                            String<uint64_t> & set,
    ...
    for (unsigned k = read_str; k < read_end; k++)
    {
        ...
        //if (++dt == 1)
        {
            ...
            if(hashNextX(shape, begin(read) + k) ^ xpre)
            {
                ...
                {
                    //double t1 = sysTime();
                    uint64_t val = index.getVal(shape.XValue, i);
                    uint64_t cordy = k;
 ...
 */
 unsigned getHIndexMatchAll(LIndex & index,
                           ...
                           PMPParms & pm_pmp)
{   
    GetHIndexMatchAllParms & pm_ghima = pm_pmp.pm_ghima;
    int dt = 0;
    ...
    for (unsigned k = read_str; k < read_end - shape.span; k++)
    {
        ...
        if (++dt == pm_ghima.thd_alpha)
        {
            ...
            if(hashNextX(shape, begin(read) + k) ^ xpre)
            {
                ...
                if (index.isEmptyDir(pos) || ptr >= (uint64_t) pm_ghima.thd_delta)
                {
                    dt = 0;
                    continue;
                }
...
uint64_t binningFilter(String<uint64_t> & anchors)
{
    String<uint64_t> bins_pointer;
    String<uint64_t> bins_counter;
    String<uint64_t> accept_bin_dir;
    uint64_t thd_accept_bin = 10;
    unsigned bin_num = 10000;
    uint64_t bin_size = 30000; 
    resize (bins_counter, bin_num, 0);
    resize (bins_pointer, length(anchors));
    for (unsigned i = 0; i < length(anchors); i++)
    {
        uint64_t bin_i = get_cord_x(anchors[i]) / bin_size;
        bins_pointer[i] = bin_i;
        if (bin_i > length(bins_counter))
        {
            resize (bins_counter, bin_i * 1.5);
        }
        ++bins_counter[bin_i];
    }
    unsigned ii = 0;
    for (unsigned i = 0; i < length(anchors); i++)
    {
        if (bins_counter[bins_pointer[i]] > thd_accept_bin) 
        {
 ...
 */
uint64_t filterAnchors2(Anchors & anchors, uint64_t shape_len, uint64_t thd_anchor_accept_density, uint64_t thd_anchor_accept_min, unsigned thd_anchor_err_bit, uint64_t thd_max_anchors_num, uint64_t thd_anchor_accept_err)
{    
    //double t1 = sysTime();
    if (anchors.length() <= 1)
    {
    ...
    for (unsigned i = 0; i < anchors.length(); i++) 
    {
        ...
        {
            uint64_t d_anchor = std::abs(int64_t(get_cord_x(anchors[i]) - anchors_xmean[j]));
            if (d_anchor < thd_anchor_accept_err)
            {
                anchors[ii] = anchors[i];
...
uint64_t filterAnchors(Anchors & anchors, 
                       uint64_t shape_len, 
                       uint64_t thd_anchor_accept_density, 
                       uint64_t thd_anchor_accept_min, 
                       unsigned thd_anchor_err_bit, 
                       uint64_t thd_max_anchors_num, 
                       uint64_t thd_anchor_accept_err, 
                       int alg_type)
{
    //printAnchors (anchors.set, "fas1");
    binningFilter(anchors.set);
    if(alg_type == 1)
    {
...
uint64_t getDAnchorList(Anchors & anchors, 
                        String<int64_t> & list, 
                        uint64_t read_str, 
                        uint64_t read_end, 
                        GlobalParms & pm_g)
{
    ...
    int64_t thd_anchor_accept_lens = thd_anchor_accept_lens_rate * (read_end - read_str);
    if (anchors.length() <= 1)
    {
        return 0;
    }
    ...
    for (unsigned k = 1; k < anchors.length(); k++)
    {
        ...
        if (f_continuous)
        {
            ...
            dy = std::min(std::abs(dy), int64_t(pm_g.shape_len));
            c_b += dy; 
...
uint64_t getDHitList(String<uint64_t> & hits, String<int64_t> & list, Anchors & anchors, PMPParms & pm_pmp)
{
    GetDHitListParms & pm_gdl = pm_pmp.pm_gdl;
    uint64_t mask = (1ULL << 20) - 1;
    if (!empty(list))
    {
        std::sort (begin(list), end(list), std::greater<uint64_t>());
        int tmp = (int) length(list) > pm_gdl.thd_list_n ? pm_gdl.thd_list_n : length(list);
        int record_num = 1;
        for (int k = 0; k < tmp; k++)
        {
            if (record_num > pm_gdl.thd_best_n)
            {
                break;
            }
...
GetDHitListParms::GetDHitListParms()
{
    appendValue(thd_list_ns, 10);
    appendValue(thd_list_ns, 20);
    appendValue(thd_best_ns, 999);
    appendValue(thd_best_ns, 1);
    toggle (1);
}
void GetDHitListParms::toggle(int i)
{
    if (i < (int)length(thd_list_ns) && i >= 0)
    {
        thd_list_n = thd_list_ns[i];
        thd_best_n = thd_best_ns[i];
    }
}

uint64_t getDAnchorMatchList(Anchors & anchors, String<uint64_t> & hits, uint64_t read_str, uint64_t read_end, 
                     ...
                     String<UPair> & str_ends_p, float thd_cover_len_rate = 0.5)
{
    ...
    for (unsigned i = 1; i < length(str_ends_p); i++)
    {
        ...
        if (!f_omit && i_move)
        {
            for (unsigned k = str_ends_p[i].first; k < str_ends_p[i].second; k++)
            {
...
int preFilterChains2(String<uint64_t> & hits,  String<UPair> & str_ends_p, void (*setEndFunc)(uint64_t &), uint64_t (*getCordXY)(uint64_t))
{
    ...
    for (unsigned i = 0; i < length(xycuts); i++)
    {
        uint64_t cuty = getCordXY(hits[(xycuts[i] & (~mask))]);
        for (unsigned j = 0; j < length(xy_strs) && xy_strs[j] < length(hits); j++)
        {
            if (cuty < getCordXY(hits[xy_strs[j]])){
                continue;
            }
            for (unsigned k = xy_strs[j]; k < str_ends_p[j].second; k++)
            {
                if (xycuts[i] & mask)
                {
...
int chainAnchorsHits(String<uint64_t> & anchors,
                     String<uint64_t> & hits,
                     String<int> & hits_chains_score,
                     PMPParms & pm_pmp)
{
    //double t0 = sysTime();
    ...
    //double t1 = sysTime();
    chainAnchorsBase(anchors, anchors_chains, hits_chains_score, 0, length(anchors), pm_cah.thd_chain_depth, 
       pm_cah.thd_chain_dx_depth, pm_cah.thd_best_n, pm_cah.thd_stop_chain_len_ratio, chn_score, &getAnchorX);
    //t1 = sysTime() - t1;
                  ...
                  PMPParms & pm_pmp)
{
    ...
    for (unsigned i = 1; i < length(hits); i++)
    {
        ...
        if (!is_cord_block_end(hits[i - 1]))
        {
            score += std::abs(dxy - pre_dxy);
            if (get_cord_y(hits[i]) - get_cord_y(hits[i - 1]) < 400)
            {
        ...
        {
            n_block++;
            //y_str = get_cord_y(hits[i]);
        }
        n++;
        if (is_cord_block_end(hits[i]))
        {
    ...
    int64_t max_score = 1;
        for (int i = 0; i < (int) length(cords_info); i++)
        {
            if (max_score < cords_info[i].score)
            {
                max_score = cords_info[i].score;
            }
        }
        for (int i = 0; i < (int) length(cords_info); i++)
        {
