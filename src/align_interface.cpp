...
int const flag_clip_uset = 0;
int const flag_clip_head = 1;
int const flag_clip_tail = 2;
int const flag_clip_lock = 1 << 31;
...
typename GapRecords::TRPair & GapRecords::get_r1_pair(int i){return r_pairs[i * 2];} //get rows of first(start) cord of i_th gaps
typename GapRecords::TRPair & GapRecords::get_r2_pair(int i){return r_pairs[i * 2 + 1];} //get rows of second(end) cord of i_th gaps
int GapRecords::getBamSegIdHead (int i){return bam_segs_id[i];}
int GapRecords::getBamSegIdTail (int i){return bam_segs_id[i] + 1;}
int GapRecords::get_clip_flag (int i){return clip_flags[i];}
int GapRecords::_set_clip_flag(int clip_flag, int pos)
{
    if (empty(clip_flags))
        return 1;
    int p = pos;
    ...
    if (clip_flags[p] & flag_clip_lock)
    {
        return 1;
    }
    int value = clip_flag ;
...
int GapRecords::set_clip_head_flag ()
{

}
int GapRecords::append_unset_flag()
{
    appendValue(clip_flags, unset_bit);
}
                 ...
                 )
{
    ...
    std::cout << "[]::insertGaps " << get_cord_y(back(gaps.c_pairs).first) << " " << get_cord_y(back(gaps.c_pairs).second) << "\n";
    gaps.dx = dx_;
    gaps.dy = dy_;
    return flag;
}
...
//TODO!!::clip the fist and last segment
    ...
    for (int i = d_w + 1; i < last_ ; i++)
    {
        ...
        std::cout << "clip_gaps_segs::tri " << i << " " << d_score << " " << d_score_left << " " << d_score_right << "\n";
             ...
             i == last_ - 1) 
        {
            ...
            float dens_ = (float)(buffer[bp] - buffer[prebp]) / (bp - prebp + 1);
            std::cout << "clip_gaps_segs::tri <<<<<<<<<<<<<<<,," << i << " " << d_score << " " << dens_ <<"\n";
              ...
              )
{
    ...
    clear(crv_gaps_src1);
    clear(crv_gaps_src2);
    std::cout << "xxyclip " << length(clip_records) << " " << get_cord_strand(gap_start_cord) << " " << get_cord_y(gap_start_cord) << " " << get_cord_x(gap_start_cord) << "\n";
    if (clip_ends == 0)
    {
        return 1;
    }
    ...
    printRows(row_jh1, row_jh2);
    printRows(row_jt1, row_jt2);
    std::cout << "bam_check3 " << flag_h << " " << flag_t << " " << bam_id_seg_h << " " << clippedEndPosition(row_jh2) - clippedBeginPosition(row_jh2) << " " << bam_id_seg_t << " " << clippedEndPosition(row_jt2) - clippedBeginPosition(row_jt2) << " " ;
    if (!flag_h && !flag_t)
    {
    ...
    int strand = get_cord_strand(gap_start_cord);
    ...
    for (int j = clip_starts; j < clip_ends; j++)
    {
        ...
        g_id = get_cord_id(gap_start_cord);
    ...
    std::cout << "xxyclip " << length(clip_records) << "\n";
               ...
               )
{
    ...
    int g_id = get_cord_id(gap_start_cord);
    ...
    */
    ...
    std::cout << "[]::align_gaps 2 " 
              ...
              << block_size << " "
              << get_cord_strand(gap_start_cord) << " "
              << band << " "
              << "\n"; 
    ...
    if (clip_flags == 2) //clip end of joint_tail cord. In some case jt cord tail is not be clipped when inserted to the gaps.
    {
        clip_segs(gaps.get_r2_pair(i).first,
                  gaps.get_r2_pair(i).second,
                  gaps.getJointTailCord,
                  gap_parm,
                  1);
    }
    std::cout << "xclip " << length(clip_records) << gaps.getBamSegIdHead(i) << " " << gaps.getBamSegIdTail(i) << "\n";
    //<<<<debug
        //merge_flag = 0;
    //>>>>debug
    if (merge_flag)
...
//>>debug
        ...
        {
            std::pair<int, int> lens = countCigar(bam_records[j].cigar);
            std::cout << "bam_check2 " << j << " " <<lens.first << " " << lens.second << "\n";
        }
//<<debug
...
/*
set_cigar_soft_clip
                    //int n = length(read) * strand - _nStrand(strand) * (get_cord_y(cords[i]) + endPosition(row(aligner, ri + 1)));
                    //appendValue(back(bam_records).cigar, CigarElement<>('S', n));

                resize(bam_records, length(bam_records) + 1);
                //int n = length(read) * strand - _nStrand(strand) * (get_cord_y(cords[i]) + beginPosition(row(aligner, ri + 1)));
                //appendValue(back(bam_records).cigar, CigarElement<>('S', n));
                back(bam_records).flag = (back(bam_records).flag & (~16)) | (get_cord_strand(cords[i]) << 4); 
*/
/*
 * debug utility
 */
        ...
        */
        ...
        if (flag) //merge failed:= 1.reverse strand or 2.gaps
        {
            ...
            //TODO::change dx, dy, for joint not well mapped cord, it's incorrect
                int flag_overlap = insertGaps(gaps, 
            ...
            else if (flag & 2) //case1: reverse strand no gaps
            {
                ...
                clip_segs(row(aligner, ri_pre), row(aligner, ri_pre + 1), pre_cord_start,
                          _gap_parm, 1);
                if (isClippedFlag(flag_clip_pre))
                {
                ...
                //>>debug
                clip_segs(row(aligner, ri), row(aligner, ri + 1), cord_start, _gap_parm, -1);
                setClipTailFlag(flag_clip_pre);
                setClipHeadFlag(flag_clip);
        ...
        else 
        {
            setClipTailFlag(flag_clip_pre);
            setClipHeadFlag(flag_clip);
            gaps.set_clip_flag(clip_pre_done, -1);
            std::cout << "clip_done_flag " << i + 1<< " " << clip_pre_done << " " << clip_done << "\n";
            if (pre_cord_start != emptyCord && clip_pre_done == 3)
            {
                insertBamRecordCigar(back(bam_records), 
                            row(aligner, ri_pre), 
                            row(aligner, ri_pre + 1));
