            ...
            if ((std::abs(anchor_len / (float)(g_hs_anchor_getY(anchor[k - 1]) - g_hs_anchor_getY(anchor[prek]))) > g_thd_anchor_density && anchor_len > 2) || anchor_len > g_thd_anchor)
            {
                ...
                for (int j = prek + 1; j < k; j++)
                {
                         ...
                         g_hs_anchor_getY(anchor[j]) > prey + thd_tileSize))
                    {
                        ...
                        std::cout << "gmas3 " << length(tiles) << " " << get_cord_y(back(tiles)) << " " << get_cord_x(back(tiles)) << " " << get_cord_strand (back(tiles)) << " " << gs_start << "\n";
                        kcount=0;
                    }
                    else
                    {
                                ...
                                )
{
    ...
    for (int k = 0; k < anchor_end + 1; k++)
    {
            ...
            thd_err_rate * std::max(thd_min_segment, d))
        {
            ...
            std::cout << "gmas5 " << thd_anchor_accpet << " " << prek << "\n";
    ...
    for (int i = p1; i < p2; i++) 
    {
        ...
        for (int j = p2; j < k; j++) 
        {
                ...
                && dx < dx_upper && dx > dx_lower)
            {
                ...
                std::cout << "[]::ccas_ " << g_anchor_end << " " << anchor_x << " " << anchor_y << " " << dx << " " <<  d_anchor << " |dx| band_level " <<  dx_upper << " " << dx_lower << " " << g_hs_anchor_getY(g_anchor[g_anchor_end - 1]) <<"\n";
            }
        }   
    }
    return g_anchor_end;
                    ...
                    int thd_merge_drop)
{
    ...
    int i_str = 0;    
    int i_end = it_end2 - it_str2;
    int n = 0;
    //step 1. 
    //stream gap pattens along two seqs and collect anchor
    ...
    for (uint64_t i = i_str; i < i_end; i++) //scan the read
    {
        ...
        std::cout << "ccr1 " << j_end - j_str << " " << hs_len1 << "\n";
        tzs[0] = ctzb_4_(dhash);
        lzs[0] = clzb_4_(dhash);
        int m = 1;
        for (uint64_t j = j_str + 1; j < j_end; j++) //scan the genome
        {
            dhash = hash_read ^ hashs[j];
            tzs[m] = ctzb_4_(dhash);
            lzs[m] = clzb_4_(dhash);
            if (c_isGapMatch_2anchor_(anchors[n], j, i, dhash, tzs[m - 1], tzs[m], lzs[m - 1], lzs[m], c_shape_len3))
            {
                n++;
            }
            m++;
        ...
        std::cout << "ccr2 " << n << "\n";
    //step2.
    //merge anchors and extend regions that can be mapped towards the breakpoint 
    if (n == 0) //empty anchor
    {
        return 0; 
    }
    String <short> pre_x, tmp_x;
    String <short> pre_y, tmp_y;
    String <int> its;
    resize (pre_x, length(tzs)); //extend kmer coordinates x
    resize (pre_y, length(tzs));
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        ...
        else // scan blocks in the range.
        {
            if (i == its[itsk + 1])//reach end of current block
            {
                ...
                {
                    if (itsk < 1 ) //if it's the last block of the current range
                    {
...
int c_clip_extend_right(uint64_t & ex_d, // results
                    String<uint64_t> & hashs, 
                    String<uint64_t> & anchors,
                    String<Dna5> & seq1, 
                    String<Dna5> & seq2, 
                    ...
                    int thd_drop)
{
    ...
    Iterator<String<Dna5> >::Type it_end1 = begin(seq1) + get_cord_x(extend_end);
    Iterator<String<Dna5> >::Type it_str2 = begin(seq2) + get_cord_y(extend_str);
    Iterator<String<Dna5> >::Type it_end2 = begin(seq2) + get_cord_y(extend_end);
    int hs_len1 = it_end1 - it_str1;
    int hs_len2 = it_end2 - it_str2;
    if (length(hashs) < hs_len1 ||
        hs_len1 < c_shape_len3  || 
        hs_len2 < c_shape_len3)
    {
        return 1;
    }
    ...
    for (uint64_t i = i_str; i < i_end; i++) //scan the read
    {
        ...
        for (uint64_t j = j_str + 1; j < j_end; j++) //scan the genome
        {
            ...
            tzs[m] = ctzb_4_(dhash);
            lzs[m] = clzb_4_(dhash);
         std::cout << "ccle2 " << j << " " << i << " " << j_str << " " << j_end << "\n";
            if (c_isGapMatch_2anchor_(anchors[n], j, i, dhash, tzs[m - 1], tzs[m], lzs[m - 1], lzs[m], c_shape_len3))
            {
                short xn = j;
                short yn = i;
                bool f_first = true;
                std::cout << "ccle1xxxxxxx" << j << " " << i << " " << k_str << " " << length(chain_y) << "\n";
                if (i < thd_init_chain_i_num)
                {
                    if (std::abs(yn - xn) < thd_init_chain_da)
                    {
                        appendValue(chain_x, xn);
                        appendValue(chain_y, yn);
                        f_extend = true;
                    }
                }
                else 
                {
                    ...
                    {
                        short dx = xn - chain_x[k];
                        short dy = yn - chain_x[k];
                        short da = dx - dy;
                        if (dy <= c_shape_len3 && f_first) 
                        {
                            k_str = k;
                            f_first = 0;
                        }
 ...
 */
 int c_clip_extend_(uint64_t & result_val, 
                    String<uint64_t> & hs, 
                    String<uint64_t> & anchors,
                    IterStrD5 it_str1,  //genome
                    IterStrD5 it_end1, 
                    IterStrD5 it_str2,  //read
                    IterStrD5 it_end2, 
                    ParmClipExtend & parm,
        ...
        )
{
    ...
    if (clip_direction < 0)
    {
        extend_str = shift_cord (clip_end, -dx, -dy);
        extend_end 
    }
    else if (clip_direction > 0)
    {
        breakpoint = shift_cord (clip_str, dx, dy);
    }
    */
    ...
    if (sv_exists)
    {
        for (unsigned i = 0; i < length(sv_flags) - 1; i++)
        {
            ...
            if ((sv_flags[i] & g_sv_r) && (sv_flags[i + 1] & g_sv_l)) 
            {
                ...
                std::cout << "gag44 " << get_tile_y(clip_str) << " " << get_tile_y(clip_end) << " " << get_tile_x(clip_str) << " " << get_tile_x(clip_end) << " " << get_tile_strand(clip_str) << " " << get_tile_strand(clip_end) << "\n";
           ...
           )
{
    ...
    for (unsigned i = 1; i < length(cords); i++)
    {
        ...
        if (_DefaultCord.getCordX(cord2 - cord1) > thd_cord_gap ||
