...
//            dout << "mins" << thd_dxy_min << thd_da_zero << "\n";
            dout << "mins" << get_cord_y(tiles[i-1]) << get_cord_y(tiles[i]) << "\n";
        if (is_diff_anchor (tiles[i - 1], tiles[i], 1, thd_dxy_min, thd_da_zero))  //try dup for every ins 
        {
...
uint64_t clip_tile (String<Dna5> & seq1,
                    ...
                    String<uint64_t> & g_hs,
                    String<uint64_t> & g_hs_anchor,
                    uint64_t tile, int const sv_flag, int tile_size)
{
    CmpInt64 g_cmpll;
 ...
 */
int clip_gap_block_(String<Dna5> & seq1,
                    String<Dna5> & seq2,
                   ...
                   )
{
    ...
    for (int i = 1; i < length(tiles); i++)
    {
        ...
        if (is_diff_anchor (tiles[i - 1], tiles[i], -1, thd_dxy_min, thd_da_zero) && 
            !get_cord_strand(tiles[i - 1] ^ tiles[i]))   //del
        {
            if (direction == g_map_rght && i == length(tiles) - 1)
            {
            //    clip_tile(tiles[i - 1], g_sv_r);
            }
            else if (direction == g_map_left && i - 1 == 0)
            {
           //     clip_tile(tiles[i], g_sv_l);
            }
            else
            {
...
int clip_gap_(String<Dna5> & seq1,
                   String<Dna5> & seq2,
                   String<Dna5> & comstr, //complement reverse of the read (seq2)
                   StringSet<FeaturesDynamic > & f1,
                   StringSet<FeaturesDynamic > & f2,
    ...
    else
    {
        for (int i = 0; i < length(tiles); i++) 
        {
            ...
            if (is_cord_block_end(tiles[i]) || i == length(tiles) - 1)
            {
                ...
                clip_gap_block_(seq1, seq2, comstr, 
                                f1,
                                f2,
                                tmp_tiles, 
                                clips, 
             ...
             )
{
    ...
    dout << "mg_" << get_cord_y(gap_str) << get_cord_y(gap_end) << "\n";
    clear(tiles);
    if (get_cord_strand(gap_str ^ gap_end))
    {
        //todo::also needs map
