            ...
            )
{
    ...
    for (int i = 0; i < length(gaps.c_pairs); i++)
    {
        ...
        uint64_t cr_cord = cmpRevCord(gap_start_cord, length(read));
        ailign_cord (row(aligner, 0), 
                    row(aligner, 1), 
                    genomes[g_id], 
                    read, 
                    comrevRead, 
                ...
                ) 
{
    ...
    for (int i = 1; i < (int)length(cords); i++)
    {
        ...
        std::cout << "[]::align_cords_1 " << i << " " << flag << " " << get_cord_y(cords[i]) << "\n";
        if (flag)
        {
